 ...
 */
-(void)shake{
    CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"transform.translation.x"];//创建核心动画//transform.scale是缩放
    animation.toValue = [NSNumber numberWithFloat:10.0f];//动画末值
    animation.duration = 0.5;//一次动画时间
    animation.repeatCount = CGFLOAT_MAX;//循环播放次数
    animation.autoreverses = YES;//动画方式返回
    animation.fillMode = kCAFillModeForwards;
    [self addAnimation:animation forKey:@"shake"];//开始动画
 ...
 */
-(void)scale{
    CABasicAnimation *animation = [CABasicAnimation animationWithKeyPath:@"transform.scale"];//创建核心动画//transform.scale是缩放
    animation.toValue = [NSNumber numberWithFloat:0.80f];//动画末值
    animation.duration = 0.1;//一次动画时间
    animation.repeatCount = 1;//循环播放次数
    animation.autoreverses = YES;//动画方式返回
    animation.fillMode = kCAFillModeForwards;
    [self addAnimation:animation forKey:@"scale"];//开始动画
}
...
-(void)boom{
    ...
    for(int i = 0 ; i < cellCount ; i++){
        for(int j = 0 ; j < cellCount ; j++){
            CGFloat pWidth = MAX
            (self.frame.size.width, self.frame.size.height)/cellCount;
