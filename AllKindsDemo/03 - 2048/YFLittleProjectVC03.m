...
- (void)viewDidLoad{
    ...
    [label moveHereWithRow:1 line:4];
        ...
        for (YFLabel *label in self.chesses) {
            ...
            switch (direction) {
                case UISwipeGestureRecognizerDirectionRight:
                    [self move_next_withLabel:label withDirection:Right];
                    break;
                case UISwipeGestureRecognizerDirectionLeft:
                    [self move_next_withLabel:label withDirection:Left];

                    break;
                case UISwipeGestureRecognizerDirectionUp:
                    [self move_next_withLabel:label withDirection:Up];

                    break;
                case UISwipeGestureRecognizerDirectionDown:
                    [self move_next_withLabel:label withDirection:Down];
...
-(void)move_next_withLabel:(YFLabel*)label withDirection:(Direction_type)type{
    
    int line;
    int row;
    
    int tmp_line = label.line;
    int tmp_row = label.row;
    ...
    BOOL isNextNum = [self isHereHaveNumWithRow:row line:line];
    ...
    if (isNextNum) {  // 有数字  那么 self.tempChesses 对应的位置也有数字
        ...
        YFLabel *nextLabel = [self getLabelInArrar:self.chesses row:row line:line];
        ...
        // 有一个不是, 就说明不是所有的
        _isAllChessesNextHaveNum = NO;
        [label moveHereWithRow:row line:line];
        YFLabel *tempLabel = [self getLabelInArrar:self.tempChesses row:tmp_row line:tmp_line];
        [tempLabel moveHereWithRow:row line:line];
...
- (void)makeContentsView{
    ...
    gameName.center = CGPointMake(screen_wid * 0.5, 20 + topBtnH *0.5);
    ...
    setting.frame = CGRectMake(screen_wid - topBtnW, 20, topBtnW, topBtnH);
    [self.view addSubview:setting];
    
    UIView *line = [[UIView alloc] init];
    line.backgroundColor = [UIColor blackColor];
    line.alpha = 0.1;
    line.frame = CGRectMake(0, CGRectGetMaxY(setting.frame) , screen_wid, 1);
    ...
    nowScoreBtn.center = CGPointMake(screen_wid * 0.5, revoke.center.y);
    ...
    maxScoreBtn.frame = CGRectMake(screen_wid - padding - bottomBtnW, revoke.frame.origin.y, bottomBtnW, bottomBtnH);
...
- (void)makeCheckerboardWithMaxY:(CGFloat)boardY{
    ...
    boardBgView.frame = CGRectMake(0, boardY, screen_wid, screen_wid);
...
- (void)revokeBtnClick{
    ...
    // 改变棋子数据
    NSLog(@"当前棋子个数:%lu", (unsigned long)self.chesses.count);
